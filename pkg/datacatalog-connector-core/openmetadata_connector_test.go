...
var _ = Describe("Vault and Data Catalog connector", Ordered, func() {
	var logger zerolog.Logger
	var err error
	var n int
	utils.InitHTTPClient(&logger)
	BeforeEach(func() {
		logger = logging.LogInit(logging.CONNECTOR, "Data Catalog Connector Tests")
		logger.Trace().Msg("Setting up Data Catalog Connector test suite")
		...
		mockDataCatalogServer = createMockDataCatalogServer()
	...
	Describe("Connector creating assets and getting their info", Ordered, func() {
		...
		var servicer DataCatalogAPIServicer
		var assetID string
		var ctx context.Context
		It("should create an asset and receive a valid response", func() {
			conf := make(map[string]interface{})
			conf["datacatalog_endpoint"] = mockDataCatalogServer.URL
			conf["vault"] = vaultConf
			servicer = NewDataCatalogAPIService(conf, nil, &logger)
