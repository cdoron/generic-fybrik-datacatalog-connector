...
const TestDatabaseService = "datacatalog-s3"
const TestDataFormat = "csv"
const TestDestinationAssetID = "assetID"
const TestDestinationCatalogID = "datacatalog"
...
var mockDataCatalogServer *httptest.Server
...
// mock Data Catalog server. This table is later enriched using the patchTable() method
func constructTableStruct(assetInfo map[string]interface{}) (*client.Table, bool) {
	...
	for i := range requestColumns {
		_, ok := utils.InterfaceToMap(requestColumns[i], &logger)
		if !ok {
			return nil, false
		}
...
func patchTable(table *client.Table, patchArray []interface{}) bool {
...
// handle mock Data Catalog server GET requests
func handleGetMockDataCatalogServer(r *http.Request) (map[string]interface{}, int) {
...
// handle mock Data Catalog server POST requests
func handlePostMockDataCatalogServer(r *http.Request,
...
// returns a mock Data Catalog server, which handles select GET, POST, PUT, and PATCH requests
func createMockDataCatalogServer() *httptest.Server {
	By("opreating a mock Data Catalog server")
	svr := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet {
			By("receiving a Get request to the Data Catalog server")
			response, statusCode = handleGetMockDataCatalogServer(r)
			...
			By("receiving a Post request to the Data Catalog server")
			response, statusCode = handlePostMockDataCatalogServer(r, requestMap)
			Expect(statusCode).ToNot(Equal(http.StatusInternalServerError))
			if statusCode != 0 {
				w.WriteHeader(statusCode)
				return
			...
			By("receiving a Put request to the Data Catalog server")
			Expect(r.RequestURI).To(Equal("/v1/metadata/types/1"))
			response = map[string]interface{}{}
		} else if r.Method == http.MethodPatch {
			By("receiving a Patch request to the Data Catalog server")
